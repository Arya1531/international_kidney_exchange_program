...
int main() {
	...
	bool initial_nucl = false; // true: nucleolus; false: shapley
	bool lex_min = false;
	...
	for (N = 9; N < 10; ++N) {
		...
		for (inst = 0; inst < 100; ++inst) {
			...
			string line;
			ifstream inp;
			inp.open("/home3/kllg37/genxml-" + to_string(inst) + ".xml"); // 1 out of the 100 instances generated by William Pettersson's web tool: https://wpettersson.github.io/kidney-webapp/#/
			getline(inp, line);
			inp.close();
			...
			string line_seed;
			ifstream seed_doc;
			seed_doc.open("/home3/kllg37/seeds/seeds/n" + to_string(N) + "inst" + to_string(inst) + ".txt");
		...
		ofstream res;
		res.open(version + "/" + solution_concept + "results.txt", ofstream::out | ofstream::trunc);
