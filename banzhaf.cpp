...
int main() {
	...
	map<int, map<int, int>> cycle_dis_lexmin_c_adjusted_period;
	...
	vector<double> relative_d1_N_adjusted(12, 0);
	...
	vector<double> M_N_lex_min_c_adjusted(12, 0);
	vector<double> core_dis_N(12, 0);
	vector<double> core_dis_N_adjusted(12, 0);
	vector<double> core_dis_N_c(12, 0);
	vector<double> core_dis_N_arbitrary(12, 0);
	vector<double> core_dis_N_arbitrary_adjusted(12, 0);
	vector<long> out_of_core(12, 0);
	vector<long> out_of_core_adjusted(12, 0);
	vector<long> out_of_core_c(12, 0);
	vector<long> out_of_core_arbitrary(12, 0);
	vector<long> out_of_core_arbitrary_adjusted(12, 0);
	...
	vector<double> solution_concept_time_d1_c_adjusted_N(12, 0);
	vector<double> solution_concept_time_lexmin_N(12, 0);
	vector<double> solution_concept_time_lexmin_c_N(12, 0);
	vector<double> solution_concept_time_lexmin_c_adjusted_N(12, 0);
	...
	for (N = 4; N < 5; ++N) {
		...
		double relative_lexmin_c_adjusted_0 = 0;
		...
		for (inst = 0; inst < 1; ++inst) {
			...
			seed_doc.open("C:/Users/kllg37/source/repos/ConsoleApplication1/ConsoleApplication1/seeds/seeds/n" + to_string(N) + "inst" + to_string(inst) + ".txt");
			...
			//--------------------d1+c--------------------------
			cout << "start minimizing d1_c" << endl;
			y_core_dist = 0;
			s_core_dist = 0;
			c_involved = true;
			period_0(Q, no_of_active_nodes, N, s, Vp, node_arrives, active_nodes, active_nodes_original, c, c_b, c_original, credit);
			...
			period_0(Q, no_of_active_nodes, N, s, Vp, node_arrives, active_nodes, active_nodes_original, c, c_b, c_original, credit);
			t0 = cpuTime();
			min_d_1(node_arrives, g, g_original, arc_pair, leaving, active_nodes, active_nodes_original, c, c_b, c_original, disp, no_of_active_nodes, N, Vp, periods, dispy, s, Q, v,  credit, edge_card_weight, t0, nodeset, cycle_distri, d, M_total, d_total, c_involved, cycle_dis_d_adjusted, numofMaxSolution, arbitray_maximum, initialSize, S, core_100, negative_core, prec, init_alloc, d_c_total, inst, unique_impu, unique_imputation, core_dist, init_alloc_accum, s_accum, v_accum, y_core_dist, s_core_dist, credit_adjusted, M,lex_min, max_d, game_generation_d1_c_adjusted, solution_concept_time_d1_c_adjusted, time_d1_c_adjusted, cycle_dis_d_adjusted_period, cycle_dis_arbitrary_period);
			t1 = cpuTime();
			total_time_d1_c_adjusted += t1 - t0;
			max_d1_c_adjusted += max_d;
			credit_adjusted = false;
			c_involved = false;
			cout << "negative core: " << negative_core;
			relative_d1_adjusted += (d_c_total / M_total);
			cout << "d_total" << d_total<< endl;
			cout << "relative_d1_adjusted: " << relative_d1_adjusted << endl;
			cout << "the number of countries: " << N << " " << "relative_d1" << " " << inst << " " << relative_d1 / (inst + 1) << endl;
			M_100_adjusted += M_total;
			cout << "the number of countries: " << N << " " << "relative_d1" << " " << inst << " " << M_100 / (inst + 1);
			//core_d_adjusted += core_100;
			//negative_core_d_adjusted += negative_core;
			cout << N << "countries" << " " << "instance_" << inst << "d1* done..."<<endl;
			//--------------------lexmin-----------------------
			cout << "start minimizing lexmin" << endl;
			y_core_dist = 0;
			s_core_dist = 0;
			lex_min = true;
			period_0(Q, no_of_active_nodes, N, s, Vp, node_arrives, active_nodes, active_nodes_original, c, c_b, c_original, credit);
			...
			period_0(Q, no_of_active_nodes, N, s, Vp, node_arrives, active_nodes, active_nodes_original, c, c_b, c_original, credit);
			t0 = cpuTime();
			min_d_1(node_arrives, g, g_original, arc_pair, leaving, active_nodes, active_nodes_original, c, c_b, c_original, disp, no_of_active_nodes, N, Vp, periods, dispy, s, Q, v, credit, edge_card_weight, t0, nodeset, cycle_distri, d, M_total, d_total, c_involved, cycle_dis_lexmin, numofMaxSolution, arbitray_maximum, initialSize, S, core_100, negative_core, prec, init_alloc, d_c_total, inst, unique_impu, unique_imputation, core_dist, init_alloc_accum, s_accum, v_accum, y_core_dist, s_core_dist, credit_adjusted, M, lex_min, max_d, game_generation_lexmin_c_adjusted, solution_concept_time_lexmin_c_adjusted, time_lex_min_c_adjusted, cycle_dis_lexmin_c_adjusted_period, cycle_dis_arbitrary_period);
			t1 = cpuTime();
			total_time_lex_min_c_adjusted += t1 - t0;
			relative_lexmin_c_adjusted_0 += (d_c_total / M_total);
			credit_adjusted = false;
			lex_min = false;
			c_involved = false;
			M_lex_min_c_adjusted += M_total;
			max_lexmin_c_adjusted_0 += max_d;*/

		}
		relative_d1_N[N - 4] = relative_d1 / 100;
		relative_d1_N_adjusted[N - 4] = relative_d1_adjusted / 100;
		...
		relative_lexmin_c_adjusted[N - 4] = relative_lexmin_c_adjusted_0 / 100;
		M_N[N - 4] = M_100 / 100;
		M_N_adjusted[N - 4] = M_100_adjusted / 100;
		...
		total_time_d1_c_adjusted_N[N - 4] = total_time_d1_c_adjusted / 100;
		total_time_lex_min_N[N - 4] = total_time_lex_min / 100;
		total_time_lex_min_c_N[N - 4] = total_time_lex_min_c / 100;
		total_time_lex_min_c_adjusted_N[N - 4] = total_time_lex_min_c_adjusted / 100;
		...
		solution_concept_time_d1_c_adjusted_N[N - 4] = solution_concept_time_d1_c_adjusted / 100;
		...
		solution_concept_time_lexmin_c_adjusted_N[N - 4] = solution_concept_time_lexmin_c_adjusted / 100;
		ofstream res;
		res.open("results_Banzhaf.txt", ofstream::out | ofstream::trunc);
		for (unsigned short int i = 0; i < N - 3; i++) {
			...
			res << "averange number of transplants d1+c*: " << M_N_adjusted[i] << endl;
			...
			res << "averange number of transplants lexmin+c*: " << M_N_lex_min_c_adjusted[i] << endl;
			...
			res << "total time d1+c*: " << total_time_d1_c_adjusted_N[i] << endl;
			res << "total time lexmin: " << total_time_lex_min_N[i] << endl;
			res << "total time lexmin+c: " << total_time_lex_min_c_N[i] << endl;
			res << "total time lexmin+c*: " << total_time_lex_min_c_adjusted_N[i] << endl;
			...
			res << "scenario time d1+c*: " << time_d1_c_adjusted_N[i] << endl;
			res << "scenario time lexmin: " << time_lex_min_N[i] << endl;
			res << "scenario time lexmin+c: " << time_lex_min_c_N[i] << endl;
			res << "scenario time lexmin+c*: " << time_lex_min_c_adjusted_N[i] << endl;
			...
			res << "game generation d1+c*: " << game_generation_d1_c_adjusted_N[i] << endl;
			res << "game generation lexmin: " << game_generation_lexmin_N[i] << endl;
			res << "game generation lexmin+c: " << game_generation_lexmin_c_N[i] << endl;
			res << "game generation lexmin+c*: " << game_generation_lexmin_c_adjusted_N[i] << endl;
			...
			res << "solution concept d1+c*: " << solution_concept_time_d1_c_adjusted_N[i] << endl;
			...
			res << "solution concept lexmin+c*: " << solution_concept_time_lexmin_c_adjusted_N[i] << endl;
			res << endl;
		}
		...
		ofstream res_dis;
		res_dis.open("cycle_dis_d" + to_string(N) + ".txt", ofstream::out | ofstream::trunc);
		...
		ofstream res_dis_star;
		res_dis_star.open("cycle_dis_d_star" + to_string(N) + ".txt", ofstream::out | ofstream::trunc);
		for (const auto& elem : cycle_dis_d_adjusted) {
			res_dis_star << elem.first << ": " << elem.second << endl;
		}
		res_dis_star << endl;
		res_dis_star.close();

		ofstream res_dis_c;
		res_dis_c.open("cycle_dis_c" + to_string(N) + ".txt", ofstream::out | ofstream::trunc);
		...
		res_dis_arbitrary.open("cycle_dis_arbitrary" + to_string(N) + ".txt", ofstream::out | ofstream::trunc);
		...
		res_dis_lexmin.open("cycle_dis_lexmin" + to_string(N) + ".txt", ofstream::out | ofstream::trunc);
		...
		res_dis_lexmin_c.open("cycle_dis_lexmin_c" + to_string(N) + ".txt", ofstream::out | ofstream::trunc);
		...
		ofstream res_dis_lexmin_star;
		res_dis_lexmin_star.open("cycle_dis_lexmin_star" + to_string(N) + ".txt", ofstream::out | ofstream::trunc);
		for (const auto& elem : cycle_dis_lexmin_c_adjusted) {
			res_dis_lexmin_star << elem.first << ": " << elem.second << endl;
		}
		res_dis_lexmin_star << endl;
		res_dis_lexmin_star.close();
		...
		for (unsigned short int i = 0; i < 24; ++i) {
			res_cycle_dis_d_period.open("cycle_dis_d_period" + to_string(N) + "_" + to_string(i) + ".txt", ofstream::out | ofstream::trunc);
		...
		for (unsigned short int i = 0; i < 24; ++i) {
			res_cycle_dis_c_period.open("cycle_dis_c_period" + to_string(N) + "_" + to_string(i) + ".txt", ofstream::out | ofstream::trunc);
		...
		ofstream res_cycle_dis_c_adjusted_period;
		check_value_d_c = 0;
		for (unsigned short int i = 0; i < 24; ++i) {
			res_cycle_dis_c_adjusted_period.open("cycle_dis_c_adjusted_period" + to_string(N) + "_" + to_string(i) + ".txt", ofstream::out | ofstream::trunc);
			for (const auto& elem : cycle_dis_d_adjusted_period[(N) * (i + 1)]) {
				check_value_d_c += elem.first * elem.second;
				res_cycle_dis_c_adjusted_period << elem.first << ": " << elem.second << endl;
			}
			res_cycle_dis_c_adjusted_period << endl;
			res_cycle_dis_c_adjusted_period.close();
		}
		cout << "check_value_d_c_adjusted: " << check_value_d_c << endl;
		res_cycle_dis_c_adjusted_period.close();
		...
		for (unsigned short int i = 0; i < 24; ++i) {
			res_cycle_dis_arbitrary_period.open("cycle_dis_arbitrary_period" + to_string(N) + "_" + to_string(i) + ".txt", ofstream::out | ofstream::trunc);
		...
		for (unsigned short int i = 0; i < 24; ++i) {
			res_dis_lexmin_period.open("cycle_dis_lexmin_period" + to_string(N) + "_" + to_string(i) + ".txt", ofstream::out | ofstream::trunc);
		...
		for (unsigned short int i = 0; i < 24; ++i) {
			res_cycle_dis_lexmin_c_period.open("cycle_dis_lexmin_c_period" + to_string(N) + "_" + to_string(i) + ".txt", ofstream::out | ofstream::trunc);
		...
		ofstream res_cycle_dis_lexmin_c_adjusted_period;
		check_value = 0;
		for (unsigned short int i = 0; i < 24; ++i) {
			res_cycle_dis_lexmin_c_adjusted_period.open("cycle_dis_lexmin_c_adjusted_period" + to_string(N) + "_" + to_string(i) + ".txt", ofstream::out | ofstream::trunc);
			for (const auto& elem : cycle_dis_lexmin_c_adjusted_period[(N) * (i + 1)]) {
				check_value += elem.first * elem.second;
				res_cycle_dis_lexmin_c_adjusted_period << elem.first << ": " << elem.second << endl;
			}
			res_cycle_dis_lexmin_c_adjusted_period << endl;
			res_cycle_dis_lexmin_c_adjusted_period.close();
		}
		cout << "check_value: " << check_value << endl;
		res_cycle_dis_lexmin_c_adjusted_period.close();
	}
...
void arbitraryMaximum(vector<unsigned short int>& node_arrives, ListGraph& g, ListDigraph& g_original, vector<pair<int, int>>& arc_pair, vector<bool>& leaving, ListGraph::NodeMap<bool>& active_nodes, ListDigraph::NodeMap<bool>& active_nodes_original, vector<ListGraph::Node>& c, vector<ListGraph::Node>& c_b, vector<ListDigraph::Node>& c_original, bool& disp, vector<unsigned short int>& no_of_active_nodes, unsigned short int& N, unsigned short int& Vp, unsigned short int& periods, bool& dispy, vector<unsigned short int>& s, unsigned short int& Q, vector<double>& v,  vector<double>& credit, ListGraph::EdgeMap<double>& edge_card_weight, double& t0, vector<int>& nodeset, vector<pair<int, int>>& cycle_distri, vector<double>& d, double& M_total, double& d_total, bool& c_involved, map<int, int>& cycle_dis, map<int, int>& numofMaxSolution, bool& arbitray_maximum, unsigned short int& initialSize, unsigned int& S, double& core_100, long& negative_core, double& prec, vector<double>& init_alloc, unsigned short int inst, bool& unique_impu, bool& unique_imputation, bool &core_dist, vector<double> &init_alloc_accum, vector<double> &s_accum, vector<double> &v_accum, double& y_core_dist, double& s_core_dist, bool& credit_adjusted, double &M, double& max_d, double& game_generation, double& solution_concept_time, std::map<int, std::map<int, int>>& cycle_dis_arbitrary_period)  
{
	...
	while (Q < periods) {
		...
		/*if (core_distance(init_alloc, v, N, S) > 0) {
			cout << "core_dist: " << core_distance(init_alloc, v, N, S) << endl;
			core_100 += core_distance(init_alloc, v, N, S);
		}
		else {
			if (abs(core_distance(init_alloc, v, N, S) >prec)) {
				cout << "error_core_dist: " << core_distance(init_alloc, v, N, S) << endl;
				negative_core += 1;
				cout << "error_core_dist_arbitrary: " << core_distance(init_alloc, v, N, S) << endl;
				std::string fileName = "Error_not_in_the_core.txt";
				std::ofstream outputFile(fileName);

				if (outputFile.is_open()) {
					// Write data to the file
					outputFile << N << "countries" << "," << "instance: " << inst << "Period: " << Q << "error_core_dist: " << core_distance(init_alloc, v, N, S) << endl;
	...
	/*if (core_dist) {
		if (core_distance(init_alloc_accum, v_accum, N, S) > 0) {
			y_core_dist += core_distance(init_alloc_accum, v_accum, N, S);
		}
		if (core_distance(s_accum, v_accum, N, S) > 0) {
